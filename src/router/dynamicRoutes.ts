...
export function registerAsyncRoutes(router: Router, menuList: MenuListType[]): void {
    ...
    menuList.forEach((route) => {
        if (route.path && !router.hasRoute(route.path)) {
    ...
    // 保存 iframe 路由
    useMenuStore().saveIframeRoutes(iframeRoutes)
}
...
function convertRouteComponent(route: MenuListType, iframeRoutes: MenuListType[]): RouteRecordRaw {
    ...
    if (route.meta.iframe) {
        // 处理 iframe 路由
        converted.path = `/outside/iframe/${route.meta.link}`
        // converted.component = () => import('@/views/outside/Iframe.vue')
        // 收集 iframe 路由用于后续处理
        iframeRoutes.push(route)
    } else {
        // 处理普通路由组件
        converted.component = loadComponent(component as string, route.component)
    }
